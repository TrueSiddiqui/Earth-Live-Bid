    ...
    <main className="min-h-screen bg-[#FFFFFF] flex items-center justify-center px-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="font-['Playfair_Display'] text-3xl font-bold text-[#2E3A46] mb-2">Apply for Membership</h1>
          <p className="text-[#2E3A46]/50">Applicants from all countries and continents are welcome. This account only lets you apply; membership and bidding require identity and wealth approval.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 rounded-xl bg-[#FFFFFF] border border-[#A67C00]/10 space-y-5">
          ...
          <div>
            <label className="block text-sm text-[#2E3A46]/60 mb-1.5">Full Name</label>
            <div className="relative">
              <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#2E3A46]/30" />
              <input
                type="text"
                value={fullName}
          ...
          >
            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><ArrowRight className="w-5 h-5" /> Create Applicant Account</>}
          </button>

          <p className="text-center text-sm text-[#2E3A46]/40">
            Already a member?{' '}
            <Link href="/login" className="text-[#A67C00] hover:underline">Sign In</Link>
          </p>
        </form>
      </motion.div>
    </main>
